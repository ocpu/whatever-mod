...
class TileEntityCache2 : TileEntity() {
  lateinit var master: TileEntityCache2
  val isMaster get() = master == this
  var tileToStack = mutableMapOf<TileEntityCache2, Stack>()
  ...
  fun add() {
    ...
    val masters = listOf(
        ...
        .filterIsInstance<TileEntityCache2>()
        .map(TileEntityCache2::master)
        .toSet()
...
//      world.spawnEntity(items)
    ...
    val foundMaster = mutableMapOf<TileEntityCache2, TileEntityCache2>()
    val foundAdjacent = mutableMapOf(*adjacentCaches.map { it to mutableListOf<TileEntityCache2>() }.toTypedArray())
    ...
    val groups = mutableListOf<MutableList<TileEntityCache2>>()
